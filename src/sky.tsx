import {allConstellations, Constellation, ConstellationEdge, edgeEquals, Star,} from "./constellation";
...
function placeConstellations(constellations: Constellation[]) {
    ...
    for (let i = 0; i < 200; ++i) {
        ...
        for (const c1 of placedConstellations) {
            ...
            let centroid1 = centroid(c1.constellation.convexHull).transform(c1.matrix)
            for (const c2 of placedConstellations.filter(c2 => c2 !== c1)) {
                let polygon2 = new Flatten.Polygon(c2.constellation.convexHull).transform(c2.matrix)
                let centroid2 = centroid(c2.constellation.convexHull).transform(c2.matrix)
                if (Flatten.Relations.intersect(polygon1, polygon2)) {
                    const v1 = new Flatten.Vector(centroid2, centroid1).multiply(0.10)
...
export function makeSky(aspectRatio: number): Sky {
    ...
    for (const c of constellations) {
        box = box.merge(new Flatten.Polygon(c.constellation.convexHull).transform(c.matrix).box)
    }
    const padding = 50
